...
void HeapSort::shiftRight(int low, int high)
{
    ...
    while ((root*2)+1 <= high)
    {
        ...
        steps.push_back(new Step(swapIdx, leftChild, Step::COMP, ++numbOfSteps));
        if (this->currentTuple[swapIdx] < this->currentTuple[leftChild])
        {
            swapIdx = leftChild;
        }
        /*If right child exists check if it is less than current root*/
        steps.push_back(new Step(swapIdx, high, Step::COMP, ++numbOfSteps));
        ...
        if (swapIdx != root)
        {
            steps.push_back(new Step(root, swapIdx, Step::SWAP, ++numbOfSteps));
...
void HeapSort::sort()
{
   /* assert(arr);
    ...
    while (high > 0)
    {
        ...
        steps.push_back(new Step(high, 0, Step::SWAP, ++numbOfSteps));
		--high;
